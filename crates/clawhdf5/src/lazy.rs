...
//! [`LazyFile`] works with any [`clawhdf5_io::HDF5Read`] backend (including
...
impl LazyFile<clawhdf5_io::MemoryReader> {
    /// Open a lazy file from an in-memory byte vector.
    pub fn from_bytes(data: Vec<u8>) -> Result<Self, Error> {
        let reader = clawhdf5_io::MemoryReader::new(data);
        Self::open(reader)
    }
}

#[cfg(feature = "mmap")]
impl LazyFile<clawhdf5_io::MmapReader> {
...
) -> Result<&clawhdf5_format::object_header::HeaderMessage, Error> {
    header
        .messages
        .iter()
